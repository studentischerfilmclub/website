...
function createSingleElectionHtml(election) {
    ...
    const candidate_rows = candidates_and_votes
        .map(([candidate, votes], index) => {
            if (votes >= min_winning_votes) {
                return `<div class="vote-number">${votes}</div><div class="filmtitle"><span class="dark-background">${candidate}</span></div>`
            }
            return `<div class="vote-number">${votes}</div><div class="filmtitle">${candidate}</div>`
        })
